...
public class Manage_game implements Screen, GestureDetector.GestureListener {
    ...
    public float pos_cam_X = 0;
    public float pos_cam_Y = 0;
    public float temp_X = 0;
    public float temp_Y = 0;
    public Music music_fight = Gdx.audio.newMusic(Gdx.files.internal("music/sword_fight.mp3"));
    public float GAME_WORLD_WIDTH = 2209;
    public float GAME_WORLD_HEIGHT = 1761;
        ...
        for (int i = 0; i < al.size(); i++) {
            ...
            if (unit.recruit >= 0) {
                ...
                for (int j = 0; j < ad.size(); j++) {
                    u2 = (Unit) ad.get(j);
                    if (u2.recruit >= 0 && unit.x - u2.x < 4*32 && unit.x - u2.x > -4*32 && unit.y - u2.y < 4*32 && unit.y - u2.y > -4*32) {
                        boolean test3 = true;
                        ...
                        if (test3) {
                            batch.draw(red_select, u2.x, u2.y);
                            batch.draw(u2.img, u2.x, u2.y);
                            done.add(j);
                        }
                    }
                }
    ...
    public void select() {
        ...
        if (Gdx.input.isTouched() && !noTouch) {
            test = null;
            float x = (camera.position.x + (Gdx.input.getX() / (2 / (5/((float)Gdx.graphics.getHeight()/64)))) - (Gdx.graphics.getWidth() / (4 / (5/((float)Gdx.graphics.getHeight()/64))))) / 32;
            float y = 1 + (camera.position.y + (Gdx.graphics.getHeight() / (2.4f / (5/((float)Gdx.graphics.getHeight()/64)))) - (Gdx.input.getY() / (2 / (5/((float)Gdx.graphics.getHeight()/64)))) - (Gdx.graphics.getHeight() / (4 / (5/((float)Gdx.graphics.getHeight()/64))))) / 32;
            for (int i = 0; i < al.size(); i++) {
                unit = (Unit) al.get(i);
           ...
           /* if (type != null && type.mv == 0 && test == null) {
                unit = new Unit();
                unit.builder(type.x, type.y);
                al.add(unit);
                unit = new Unit();
                unit.swordsman(type.x, type.y);
                al.add(unit);
                unit = new Unit();
                unit.spearman(type.x, type.y);
                al.add(unit);
                unit = new Unit();
                unit.horseman(type.x, type.y);
                al.add(unit);
    ...
    public void fight(int i) {
        ...
        this.music_fight.play();
        this.music_fight.setVolume(0.5f);
    }
    ...
    private void handlenput() {
        ...
        camera.zoom = MathUtils.clamp(camera.zoom, 5/((float)Gdx.graphics.getHeight()/64), 5/((float)Gdx.graphics.getHeight()/64));
